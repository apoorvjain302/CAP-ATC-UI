...
const _TADIR_SET = new Set(require("./tadirData.json"));
const _TFDIR_SET = new Set(require("./tfdirData.json"));
...
function _classifyRow(row, migType, tadirSet, tfdirSet) {
  ...
  if (_cs("2365665") || _eqN("2535093")) {
    if (_eq("DTEL")) { remType = "Needs Remediation"; remark = "Replace data element with new or custom."; }
    else if (refObjType === "TABL" && (refObj === "MARA" || !refObj)) {
      remType = "Fit Gap"; isFitGapDelta = true; remark = "Need to check uses of seasonal field into system as per notes.";
    }
    else if (_eq("TABL")) { remType = "Fit Gap"; remark = "The season fields are no longer maintained in table MARA. But maintained in table FSH_SEASONS_MAT."; }
  }
  ...
  if (_eqN("2689873")) {
    ...
    else if (_eq("DOMA") || _eq("TTYP") || _eq("MSAG")) { remType = "Needs Remediation"; remark = "DB Objects either has been deleted or DEPRECATED"; }
  ...
  if (_eqN("2689873") && process.env.ATCDEBUG) { console.error("AFTER2689873:", remType, "remark:", remark.slice(0,40)); }
  ...
  if (_eqN("2268085") && !isLiteral) {
    if (_eq("MD04") || _eq("MD4C") || _eq("MDBS")) {
      ...
      remType = "False Positive"; remark = "Function module available in S/4HANA.";
  ...
  if (_eqN("2469385") && !isLiteral) {
    if (_cs("ISM_CUSTOMER_PI") || _cs("RJGBUSISM007") || _eq("A386") || _eq("JPTTITLERELATTR") || refObj === "TABL") {
  ...
  if (_eqN("2296016") && !isLiteral) {
    const fp1 = new Set([
      "LAST_DAY_OF_MONTHS","BSAK","BSAD","BSAS","BSID","BSIK","BSIS","CKIS",
      ...
      "BAPI1076_SPPICLASS","MARD","MARI","MCHB","MSEG",
  ...
  // KK: blanket FG. Manual: FUNC rows → NR (FM exists in S/4H with changed signature), TABL → FG.
  if (_eqN("2227532") || _eqN("2227579")) {
    if (refObjType === "FUNC" || _eq("MD_STOCK_REQUIREMENTS_LIST_API") || _eq("AUFBAUEN_MDPSX_ANZEIGEN")) {
  ...
  // Note 2368913 — flag MM (KK: ALL → 'Need to check into system as per notes')
  // ABAP WHEN 'MM': PERFORM add_analysis USING 'Need to check into system as per notes' ...
  ...
  if (_eqN("2368913")) {
    ...
    if ((refObj && refObj.startsWith("WRF_")) || (foundInTadir && !(refObj === "EKET" || refObj === "EKKO"))) {
      ...
      remType = "Need to check into system as per notes"; isFitGapDelta = true; remark = "In SAP S/4HANA, seasonal procurement is not available anymore.";
    }
  }
  ...
  if (title.includes("SEARCH FOR USAGES OF SIMPLIFIED OBJECTS") ||
    ...
    if (process.env.ATCDEBUG) { console.error("BEFORE_TADIR_GATE:", remType); }
      ...
      if (isFuncType) {
        ...
        if (tfdirSet && tfdirSet.has(fnKey)) {
          ...
          if (isUpgrade) {
            ...
            // All other messages (blank/Y/Z) → Can be ignored (object is available in S/4H).
            // NON-STRATEGIC-FUNCTION (exact, flag 'RS' in ABAP loop 1) → False Positive:
            //   ABAP sets lv_rem_ty='False Positive' in loop 1 and loop 2 does not overwrite
            //   for FUNC-found + blank lv_check (no assignment unless in hardcoded FM list).
        ...
        if (tadirSet && tadirSet.has(tadirKey)) {
          ...
          if (isUpgrade) {
            ...
            if (_isHardConflict(checkMsg)) {
              ...
              if (lookupType === "INTF" || lookupType === "CLAS") {
                remType = "Can be ignored";
                remark  = "Class/Interface Available in S/4HANA.";
              } else {
                remType = "Can be ignored";
                remark  = "Data Element, Domain or Table Type Available in S/4HANA.";
              }
            }
          }
  ...
  // SELECT/query and cursor operations keep their own remType (set earlier or default).
  // This rule must come after all note-based rules so note-specific outcomes are preserved.
  if (!remType && isS4H && isDml &&
      (title.includes("SEARCH FOR DATABASE OPERATIONS") ||
       title.includes("S/4HANA: SEARCH FOR DATABASE OPERATIONS"))) {
    remType = "Fit Gap";
    remark  = "DML operation (INSERT/UPDATE/MODIFY/DELETE) on a simplified table — functional assessment required.";
  }
  ...
  if (process.env.ATCDEBUG) { console.error("BEFORE_DEFAULT:", remType); }
  // ── Default fallback ─────────────────────────────────────────────────────
  if (!remType) {
    if (isS4H) {
  ...
  const isFitGap      = remType === "Fit Gap";
...
function classifyRows(rows, cloneBuf, migType, tadirSet, tfdirSet) {
  ...
  const _tadir    = (tadirSet instanceof Set && tadirSet.size > 0) ? tadirSet : _TADIR_SET;
  const _tfdir    = (tfdirSet instanceof Set && tfdirSet.size > 0) ? tfdirSet : _TFDIR_SET;
  ...
  // ── Post-processing: propagate Syntax Error, then Fit Gap ────────────────────
  // Order matters: Syntax Error must run first so it takes priority over Fit Gap.
  // If any row for an object is Syntax Error, ALL rows of that object become Syntax Error.
  // If any row for an object is Fit Gap (and the object is not a Syntax Error object),
  // all non-syntax rows become Fit Gap.
 ...
 * reclassify ALL rows of that same object as Fit Gap.
 * Rows already classified as Syntax Error are never overridden.
...
function propagateFitGap(rows) {
  ...
  for (const row of rows) {
    if (row["Fit Gap?"] === "Yes" && row["Syntax Error?"] !== "Yes") {
  ...
  if (fitGapObjKeys.size > 0) {
    let propagated = 0;
    for (const row of rows) {
      // Never override syntax errors
      if (row["Syntax Error?"] === "Yes") continue;
      if (row["Fit Gap?"] === "Yes") continue;

      const objKey = `${(row["Obj."] || "").toString().trim().toUpperCase()}||${(row["Object name"] || "").toString().trim().toUpperCase()}`;
      if (fitGapObjKeys.has(objKey)) {
        row["Remediation Type"] = "Fit Gap";
        row["Fit Gap?"]         = "Yes";
        row["Fit Gap Delta?"]   = "No";
        propagated++;
      }
    }
    if (propagated > 0) {
      console.log(`[atcClassify] Fit Gap propagation: ${fitGapObjKeys.size} objects, ${propagated} rows reclassified as Fit Gap`);
    }
  }
 ...
 * reclassify ALL rows of that same object as Syntax Error.
 * This mirrors propagateFitGap — objects with pre-existing errors are fully
 * out of scope, so all their rows (even Mandatory/NR) should be Syntax Error.
 * Must be called AFTER all chunks are merged (i.e. on the full row set).
 * Must be called BEFORE propagateFitGap so that Syntax Error takes priority.
 * Safe to call multiple times — idempotent.
 */
function propagateSyntaxError(rows) {
  ...
  if (syntaxObjKeys.size > 0) {
    ...
    for (const row of rows) {
      ...
      // If already Syntax Error: just ensure Fit Gap is cleared (Syntax Error takes priority).
      if (row["Syntax Error?"] === "Yes") {
        row["Fit Gap?"] = "No";
        continue;
      }

      // Propagate to rows that weren't originally Syntax Error
      row["Remediation Type"]    = "Syntax Error";
      row["Syntax Error?"]    = "Yes";
      // Clear Fit Gap flag — Syntax Error takes priority; the waterfall
      // hcaPreExisting / s4PreExisting filter requires !_fg to work correctly.
      row["Fit Gap?"]         = "No";
      propagated++;
    }
    if (propagated > 0) {
      console.log(`[atcClassify] Syntax Error propagation: ${syntaxObjKeys.size} objects, ${propagated} rows reclassified as Syntax Error`);
    }
  }
