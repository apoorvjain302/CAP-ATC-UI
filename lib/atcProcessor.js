...
async function _runJob(jobId) {
  ...
  const _toBuffer = async (val) => {
    ...
    // CAP SQLite may return an object with a buffer property
  ...
  try {
    ...
    // ── Load files ──────────────────────────────────────────────────────────
    const _cols = ["id","filename","mimeType","size","content"];
    let atcFile = null;
    if (!tuaOnly) {
      [atcFile] = await db.run(SELECT.from("atc.Files").columns(..._cols).where({ id: job.atcFileId }));
      if (!atcFile) throw new Error("ATC input file not found in database");
    }
    const _loadFile = async (id) => {
      if (!id) return null;
      const [f] = await db.run(SELECT.from("atc.Files").columns(..._cols).where({ id }));
      return f || null;
    };
    ...
    const atcBuf   = await _toBuffer(atcFile.content);
    if (!atcBuf) throw new Error("ATC file content is empty");
    const cloneBuf = cloneFile ? await _toBuffer(cloneFile.content) : null;
    ...
    if (useAppLogic) {
      ...
      if (classifiedXlsBuf) {
        await db.run(INSERT.into("atc.Artifacts").entries({
          id: cds.utils.uuid(), jobId, role: "atc_result",
          filename: `atc_classified_${_ts()}.xlsx`,
          mimeType: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
          content: classifiedXlsBuf, size: classifiedXlsBuf.length,
        }));
      ...
      for (let i = 0; i < chunkBufs.length; i++) {
        ...
        if (chunkBufs.length > 1 && apiResult.xlsBuffer) {
          ...
          await db.run(INSERT.into("atc.Artifacts").entries({
            id: cds.utils.uuid(), jobId,
            role: `atc_result_chunk_${i + 1}`,
            filename: `atc_api_chunk_${i + 1}_of_${chunkBufs.length}_${_ts()}.xls`,
            mimeType: "application/vnd.ms-excel",
            content: chunkBuf, size: chunkBuf.length,
          }));
        }
    ...
    if (useTua && hasTuaFiles) {
      ...
      const smodiRows = excelGen.readExcelRows(await _toBuffer(smodiFile.content), 24);
      const trnsRows  = excelGen.readExcelRows(await _toBuffer(trnFile.content),   11);
      const masterMap = nsOwnerFile
        ? excelGen.buildMasterMap(excelGen.readExcelRows(await _toBuffer(nsOwnerFile.content), 2))
        : {};
      ...
      await db.run(INSERT.into("atc.Artifacts").entries({
        id: cds.utils.uuid(), jobId, role: "tua",
        filename: `TUA_Analysis_${_ts()}.xlsx`,
        mimeType: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        content: tuaBuf, size: tuaBuf.length,
      }));
    }
    ...
    await db.run(INSERT.into("atc.Artifacts").entries({
      id: cds.utils.uuid(), jobId, role: "pptx",
      filename: `atc_presentation_${_ts()}.pptx`,
      mimeType: "application/vnd.openxmlformats-officedocument.presentationml.presentation",
      content: pptxBuf, size: pptxBuf.length,
    }));
    ...
    if (!tuaOnly) {
      ...
      await db.run(INSERT.into("atc.Artifacts").entries({
        id: cds.utils.uuid(), jobId, role: "estimation",
        filename: `effort_estimation_${_ts()}.xlsx`,
        mimeType: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        content: estBuf, size: estBuf.length,
      }));
    }
